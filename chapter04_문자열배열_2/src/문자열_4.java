...
public class 문자열_4 {
	...
	public static void main(String[] args) {
		...
		System.out.println("==================================================");
		//1. 순위가 가장 많이 상승한 순위 출력
		int max=0;
		for(int s=0;s<title.length;s++)
		{
			if(state[s].equals("상승"))
			{
				if(max<modify[s])
				{
					max=modify[s];
		...
		//2. 순위가 가장 많이 상승한 노래의 제목과 가수명 출력
		...
		System.out.println("==================================================");
		//3. 노래명중에 가장 긴 제목의 노래와 가수명, 앨범명 출력
		//3-1. max값 구하기 : 가장 긴 문자열이 몇인지
		max=0;
		String index="";
		for(String t:title)
		{
		...
		//3-2. max값의 인덱스값 구하기 : 해당 문자열의 위치가 어딘지
		int i=0;
		for(i=0;i<title.length;i++)
		{
			if(max==title[i].length())//같은 length라도 ()가 있어야 메소드임
			{
				break;
			}
		}
		...
		System.out.println("가장 긴 가수명 : "+singer[i]);
		System.out.println("가장 긴 앨범명 : "+album[i]);
		
		System.out.println("==================================================");
		//4. 노래중에 순위 상승한 노래명만 출력하시오
		for(i=0;i<title.length;i++)//위에서 i가 이미 선언되어서 초기화로 사용해야함
		{
			if(state[i]=="상승")
			{
				System.out.println("순위상승한 노래명 : "+title[i]);
				//여러개를 반복해서 찾아야 함으로 break 쓰면 안됨
			}
		}
		System.out.println("==================================================");
		//5. 노래중에 순위 하강한 노래갯수
		...
		System.out.println("하강한 노래갯수 : "+count);
		
		System.out.println("==================================================");
		//6. 임영웅의 노래 제목과 갯수 출력하시오
		count=0;//위에서 count가 이미 선언되어서 초기화로 사용해야함
		for(i=0;i<title.length;i++)
		{
			if(singer[i].contains("임영웅"))//equal을 사용하지 않는 이유는 가수명에 임영웅(feat....)이런식으로 되어잇는경우가 잇음
			{
				System.out.println("임영웅 노래명 : "+title[i]);
				count++;
			}
		}
		System.out.println("임영웅 노래갯수 : "+count);
		
		System.out.println("==================================================");
		
		//7. 검색어를 입력 받아서 => 검색어가 함되어있는 노래명
		...
		for(String f:title)
		{
			if(f.contains(fd))
			{
				System.out.println("검색어 포함 노래명 : "+f);
			}
		}
